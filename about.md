...
### Core Philosophy
> Jupiter is not a typical e-commerce store. It is a **premium handcrafted brand experience** — a visual journey through artisanship, universal energies, and the magic of things made by hand in Nepal. Every pixel should feel intentional, warm, and alive.
...
> **Direction:** Cosmic earthy spiritual — the intersection of the universe and the Nepali landscape. Deep space meets terracotta earth. Spirals, moonlight, copper wire, and sage.

| Role | Name | Hex | Usage |
|---|---|---|---|
| **Background** | Parchment Cream | `#FAF6EF` | Page backgrounds, cards |
| **Surface** | Warm Ivory | `#F5F0E4` | Section backgrounds, alt cards |
| **Primary Dark** | Cosmic Indigo | `#1A1040` | Headlines, navbars, footers |
| **Primary Brand** | Jupiter Gold | `#C89430` | CTAs, badges, highlights |
| **Accent 1** | Sacred Sage | `#6B8F71` | Nature/organic elements |
| **Accent 2** | Dusty Lavender** | `#9B87C3` | Spiritual/cosmic elements |
| **Accent 3** | Copper Ember | `#C4714E` | Warm earthy touches, hover states |
| **Text Primary** | Deep Plum | `#2C1F3E` | Body text |
| **Text Muted** | Mystic Mauve | `#7B6E8A` | Captions, secondary text |
| **Border** | Whisper | `#E8E2D8` | Dividers, card borders |
| **White** | Starlight | `#FDFAF5` | Buttons, highlights |

**Gradient — "Jupiter Aura":**
```css
background: linear-gradient(135deg, #1A1040 0%, #3B2A6E 40%, #6B4E9B 70%, #C89430 100%);
```

**Gradient — "Earthy Cosmos":**
```css
background: linear-gradient(180deg, #FAF6EF 0%, #F0E8D8 50%, #E8D8C4 100%);
```
...
| **Section Headings** | Fraunces | 400, 600 | Google Fonts |
| **Body / UI** | Plus Jakarta Sans | 400, 500, 600 | Google Fonts |
| **Accent / Labels** | DM Mono | 400 | Google Fonts |
...
- **Spirals & Sacred Geometry** — Use subtle spiral SVG motifs as decorative elements (section dividers, backgrounds, loading states)
- **Pinterest Masonry** — Product grids use masonry layout, not strict uniform grids
- **Slow Reveal** — Framer Motion scroll-triggered animations for every section
- **Texture** — Subtle paper/linen textures on backgrounds via CSS `background-image`
- **Whitespace** — Generous spacing; let products breathe
- **Mobile First** — Design starts at 375px and scales up
- **Micro-interactions** — Hover effects on all product cards, buttons, and links

### 2.4 Iconography

Use **Lucide Icons** (already in the React ecosystem) + custom SVG for:
- Spiral motif (brand signature)
- Jupiter planet icon
- Handmade needle/thread icon
- Nepal mountain silhouette
...
| **Shop (All Products)** | `/shop` | Full catalog with filtering + search |
| **Product Detail** | `/shop/[slug]` | Full product info + WhatsApp order |
| **Category Collection** | `/collections/[category]` | Filtered view by product type |
...
│
├── / (Home)
├── /shop (All Products — masonry grid)
│   └── /shop/[slug] (Product Detail)
│
├── /collections/[category] (Category Pages)
│   ├── /collections/wire-jewelry
│   ├── /collections/rings
│   ├── /collections/mandala-art
│   ├── /collections/moon-lamps
│   ├── /collections/dried-bouquets
│   ├── /collections/custom-gifts
...
#### /admin/settings
- Edit announcement bar text and toggle on/off
...
```
[ANNOUNCEMENT BAR]
  "✨ Free shipping on orders above NPR 1500 within KTM Valley"
...
[HERO SECTION]
  Full-screen or 90vh
  Left: Large display text — "Made with Cosmic Intention" (Cormorant Garamond 72px)
  Sub: "Handcrafted jewelry & gifts from the heart of Nepal"
  CTA: [Explore Collection] [Custom Order]
  Right: Hero image — product flat lay or artisan at work
  Background: Cosmic Indigo with subtle spiral SVG pattern
  Animation: Text fades in, image slides in (Framer Motion)

[FLOATING MARQUEE]
  Scrolling text: "Wire Jewelry • Mandala Art • Moon Lamps • Custom Gifts • Made in Nepal •"
  Background: Jupiter Gold, text: Cosmic Indigo

[SHOP BY CATEGORY]
  Heading: "Explore Our Universe"
  8 category cards in a horizontal scroll (mobile) / grid (desktop)
  Each card: Full-bleed image, category name overlay, hover: zoom + gold border

[FEATURED PRODUCTS]
  Heading: "Pieces Made with Magic"
  Admin-selectable featured products (is_featured = true)
  Pinterest-style masonry grid (3 cols desktop, 2 cols tablet, 1 col mobile)
  Each card: Image, Name, Price (if set), "View" CTA, ❤️ wishlist icon

[BRAND STORY STRIP]
  Split layout: Left = image of artisan at work | Right = short story text
...
[HOW TO ORDER]
  3-step process visualization (not boring — illustrated with icons)
  1. Browse & fall in love 🔮
  2. Click "Order via WhatsApp" 💬
  3. We craft & deliver to your door 📦

[WALL OF LOVE (PREVIEW)]
  Heading: "What they're saying ✨"
  3 featured reviews in a card carousel
  CTA: [See All Reviews]

[INSTAGRAM FEED]
  Heading: "Follow our journey @jupiterrrr_11"
  Grid of latest Instagram posts (manual embed or oEmbed)
  CTA: [Follow on Instagram]

[FOOTER]
  Logo + tagline
  Navigation: Shop, Collections, Our Story, The Craft, Custom Orders, Contact
  Social: Instagram, WhatsApp
  Nepal flag + "Handmade in Nepal 🇳🇵"
  Copyright
  Privacy Policy link
```
...
RIGHT COLUMN — INFO
  ...
  ─── Shipping ───
  Small info box: "📦 Ships within Nepal | KTM Valley: 1–2 days | Outside: 3–7 days"
...
│   │   │   ├── layout.tsx       # Navbar + Footer + AnnouncementBar
│   │   │   ├── page.tsx         # Home /
│   │   │   ├── shop/
│   │   │   │   ├── page.tsx     # /shop
│   │   │   │   └── [slug]/
...
☐ Build Layout: Navbar, Footer, AnnouncementBar
```
...
☐ Site settings panel (WhatsApp number, announcement bar, etc.)
...
| Update announcement bar | `/admin/settings` |
