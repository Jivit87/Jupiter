...
export function AdminSettingsForm({ settings }: AdminSettingsFormProps) {
  ...
  const [announcementActive, setAnnouncementActive] = useState(settings['announcement_bar_active'] === true || settings['announcement_bar_active'] === 'true');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {success && <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">Settings saved!</div>}

      <section className="space-y-4">
        <h3 className="font-heading text-xl text-primary">Contact</h3>
      ...
      <section className="space-y-4">
        <h3 className="font-heading text-xl text-primary">Content</h3>
      ...
      <section className="space-y-4">
        <h3 className="font-heading text-xl text-primary">Announcement bar</h3>
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className={labelCls}>Text</label>
            <input value={announcementBar} onChange={(e) => setAnnouncementBar(e.target.value)} className={inputCls} />
          </div>
          <label className="flex cursor-pointer items-center gap-3">
            <input type="checkbox" checked={announcementActive} onChange={(e) => setAnnouncementActive(e.target.checked)} className="h-4 w-4 rounded border-border" />
            <span className="text-sm font-medium text-primary">Show announcement bar</span>
          </label>
        </div>
      </section>
