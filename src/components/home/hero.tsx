...
export function Hero({ tagline }: { tagline?: string }) {
  return (
    <section className="relative w-full bg-gradient-to-b from-amber-50/60 via-white to-white text-black min-h-[100dvh] flex items-center pt-20 pb-12 overflow-hidden">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-12 lg:px-16 flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
        
        {/* Text Content */}
        <div className="flex flex-col items-start text-left max-w-xl z-10 w-full lg:w-1/2">
          <div className="inline-flex items-center gap-2 px-3 py-1 mb-8 rounded-full border border-[#E5E7EB] bg-[#F9FAFB] text-[11px] font-semibold uppercase tracking-widest text-[#6B7280]">
            <i className="ri-map-pin-line text-sm"></i> Handcrafted in Nepal
          </div>
          
          <h1 className="font-display text-5xl sm:text-6xl lg:text-[72px] leading-[1.05] tracking-tight mb-8">
            {tagline ? tagline : (
              <>Handcrafted with <span className="italic text-[#6B7280]">cosmic intention.</span></>
            )}
          </h1>
          
          <p className="text-lg text-[#4B5563] leading-relaxed mb-10 max-w-md">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            <Link 
              ...
              className="flex items-center justify-center gap-2 bg-amber-600 text-white px-8 py-4 text-sm font-medium hover:bg-amber-700 transition-colors rounded-sm shadow-sm"
            >
              Shop Collection <i className="ri-arrow-right-line"></i>
            </Link>
            <Link 
              href={siteConfig.links.custom} 
              className="flex items-center justify-center gap-2 bg-white text-black border border-[#E5E7EB] px-8 py-4 text-sm font-medium hover:bg-[#F9FAFB] transition-colors rounded-sm"
            >
              Custom Orders
            </Link>
          </div>
