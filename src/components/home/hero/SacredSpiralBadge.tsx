  ...
  return (
    ...
    >
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" style={{ display: 'block' }}>
        ...
        {/* Outer dotted ring */}
        <circle cx={cx} cy={cy} r="86" fill="none" stroke="#C89430" strokeOpacity="0.5" strokeWidth="0.5" strokeDasharray="1.2 5" />

        {/* Thin solid ring — outer edge of text track */}
        <circle cx={cx} cy={cy} r="80" fill="none" stroke="#C89430" strokeOpacity="0.4" strokeWidth="0.4" />

        {/* Ring with 32 tiny tick marks */}
        <circle cx={cx} cy={cy} r="76" fill="none" stroke="#C89430" strokeOpacity="0.3" strokeWidth="0.35" />
        {Array.from({ length: 32 }).map((_, i) => {
          const s = spoke(i, 32, 74, 78, 0.2, 0.25);
          return <line key={`tick-${i}`} x1={s.x1} y1={s.y1} x2={s.x2} y2={s.y2} stroke="#C89430" strokeOpacity={s.op} strokeWidth={s.sw} />;
        })}

        {/* Text-baseline ring */}
        <circle cx={cx} cy={cy} r="70" fill="none" stroke="#C89430" strokeOpacity="0.25" strokeWidth="0.3" />

        {/* Mid ring */}
        <circle cx={cx} cy={cy} r="56" fill="none" stroke="#C89430" strokeOpacity="0.28" strokeWidth="0.35" />

        {/* 16 petal dots on r=56 */}
        {Array.from({ length: 16 }).map((_, i) => {
          const angle = (i * 360) / 16 - 90;
          const rad = (angle * Math.PI) / 180;
          return <circle key={`petal-${i}`} cx={cx + 56 * Math.cos(rad)} cy={cy + 56 * Math.sin(rad)} r="0.9" fill="#C89430" fillOpacity="0.45" />;
        })}

        {/* Ring at r=48 */}
        <circle cx={cx} cy={cy} r="48" fill="none" stroke="#C89430" strokeOpacity="0.22" strokeWidth="0.3" />

        {/* Ring at r=44 */}
        <circle cx={cx} cy={cy} r="44" fill="none" stroke="#C89430" strokeOpacity="0.3" strokeWidth="0.35" />

        {/* 8 accent dots on r=44 */}
        {Array.from({ length: 8 }).map((_, i) => {
          const angle = (i * 360) / 8 - 90;
          const rad = (angle * Math.PI) / 180;
          return <circle key={`dot8-${i}`} cx={cx + 44 * Math.cos(rad)} cy={cy + 44 * Math.sin(rad)} r="1.6" fill="#C89430" fillOpacity="0.55" />;
        })}
        ...
        {[38, 34, 26, 22, 18, 14, 10].map((r, idx) => (
          <circle key={`inner-${r}`} cx={cx} cy={cy} r={r} fill="none" stroke="#C89430" strokeOpacity={0.28 - idx * 0.025} strokeWidth="0.35" />
        ))}
        ...
        {/* Center marks */}
        <circle cx={cx - 3.5} cy={cy} r="1.4" fill="#C89430" fillOpacity="0.8" />
        <circle cx={cx + 3.5} cy={cy} r="1.4" fill="#C89430" fillOpacity="0.8" />
        <circle cx={cx} cy={cy} r="0.9" fill="#C89430" fillOpacity="0.9" />

        {/* Top arched text */}
        <text fill="#C89430" fillOpacity="0.82" fontFamily="var(--font-body)" fontSize="8.2" fontWeight={600} letterSpacing="2.2">
